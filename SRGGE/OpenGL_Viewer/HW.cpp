...
void HW::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_Up) camera_.MoveV(1);
  if (event->key() == Qt::Key_Down) camera_.MoveV(-1);

  if (event->key() == Qt::Key_Left) camera_.MoveH(1);
  if (event->key() == Qt::Key_Right) camera_.MoveH(-1);

  if (event->key() == Qt::Key_Z) camera_.UpDown(-1);
  if (event->key() == Qt::Key_X) camera_.UpDown(1);

  if (event->key() == Qt::Key_W) camera_.MoveX(-5);
  if (event->key() == Qt::Key_S) camera_.MoveX(5);

  if (event->key() == Qt::Key_A) camera_.MoveY(-5);
  if (event->key() == Qt::Key_D) camera_.MoveY(5);
